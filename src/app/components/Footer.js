// src/components/layout/Footer.jsx
'use client';

import React from 'react';

const SocialIcon = ({ name }) => {
  const icons = {
    Facebook: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
        <path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.879V14.89h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89.75 0 1.5.09 2.25.26v2.73h-1.5c-1.25 0-1.5.5-1.5 1.5V12h2.75l-.35 2.89h-2.4V22c4.781-.751 8.438-4.888 8.438-9.879z" />
      </svg>
    ),
    Instagram: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
      </svg>
    ),
    LinkedIn: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
        <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
      </svg>
    ),
    Twitter: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
        <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
      </svg>
    ),
  };
  return icons[name] || null;
};

const Footer = () => {
  const socialLinks = [
    { name: 'Facebook', href: 'https://facebook.com/tu-perfil' },
    { name: 'Instagram', href: 'https://instagram.com/tu-perfil' },
    { name: 'LinkedIn', href: 'https://linkedin.com/company/tu-empresa' },
    { name: 'Twitter', href: 'https://x.com/tu-perfil' },
  ];

  return (
    <footer className="footer-bg text-gray-300 py-12 md:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {/* Brand */}
          <div className="space-y-4">
            <div className="text-2xl font-bold text-blue-500 tracking-tight">Nex-v</div>
            <p className="text-sm leading-relaxed max-w-xs text-gray-400">
              Tu agencia digital de confianza para negocios locales.
            </p>
            <div className="flex space-x-3 pt-3">
              {socialLinks.map((social) => (
                <a
                  key={social.name}
                  href={social.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`Visita nuestro perfil en ${social.name}`}
                  className="social-btn"
                >
                  <SocialIcon name={social.name} />
                </a>
              ))}
            </div>
          </div>

          {/* Servicios */}
          <div>
            <h3 className="footer-title text-lg mb-4">Servicios</h3>
            <ul className="space-y-2">
              {['Desarrollo Web', 'Redes Sociales', 'SEO & SEM', 'Analíticas', 'Consultoría'].map(
                (item, i) => (
                  <li key={i}>
                    <a href="/#servicios" className="footer-link text-sm">
                      {item}
                    </a>
                  </li>
                )
              )}
            </ul>
          </div>

          {/* Empresa */}
          <div>
            <h3 className="footer-title text-lg mb-4">Empresa</h3>
            <ul className="space-y-2">
              {[
                { label: 'Quiénes Somos', href: '/#quienes-somos' },
                { label: 'Nuestro Equipo', href: '/#equipo' },
                { label: 'Casos de Éxito', href: '/#casos' },
                { label: 'Blog', href: '/blog' },
                { label: 'Contacto', href: '/#contacto' },
              ].map((item, i) => (
                <li key={i}>
                  <a href={item.href} className="footer-link text-sm">
                    {item.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Contacto */}
          <div>
            <h3 className="footer-title text-lg mb-4">Contacto</h3>
            <div className="space-y-2 text-sm footer-contact">
            
              <p> Madrid, España</p>
              <p>
                <a href="tel:+34900123456" className="footer-link">
                  +34 692 87 71 25 +34 692 93 06 04
                </a>
              </p>
              <p>
                <a href="mailto:hola@nex-v.com" className="footer-link">
                  Office.nexv@gmail.com
                </a>
              </p>
            </div>
          </div>
        </div>

        <div className="mt-12 pt-8 border-t border-gray-800 text-center">
          <p className="footer-copyright">
            © {new Date().getFullYear()} Nex-v. Todos los derechos reservados.
          </p>
        </div>
      </div>
    </footer>
  );
};

export default Footer;